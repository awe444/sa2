# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 384 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
...
# 2 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
...
# 3 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
...
# 5 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 7 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
...
u32 Coll_Player_Entity_Intersection(Sprite *s, CamCoord x, CamCoord y, Player *p);
# 8 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
...
# 10 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
...
# 12 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
...
# 15 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
# 1 "/home/awaszczak/git/sa2/sa1/../include/constants/sa1/char_states.h" 1
# 16 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
# 1 "/home/awaszczak/git/sa2/sa1/../include/constants/sa1/interactables.h" 1
# 17 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
# 1 "/home/awaszczak/git/sa2/sa1/../include/constants/sa1/songs.h" 1
# 18 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2
# 1 "/home/awaszczak/git/sa2/sa1/../include/constants/sa1/vram_hardcoded.h" 1
# 19 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/spikes.c" 2


typedef struct {
               SpriteBase base;
               Sprite s;
               u32 movestateBuffer[2];
} Spikes;
...
{
    struct Task *t = TaskCreate(Task_Spikes_Up, sizeof(Spikes), 0x2000, 0, ((void*)0));
    Spikes *spikes = (void *)((t)->data);
    Sprite *s = &spikes->s;
    CamCoord worldX, worldY;

    spikes->movestateBuffer[1] = 0;
    spikes->movestateBuffer[0] = 0;
    ...
    s->graphics.dest = ((void *)&VRAM[0x10000] + 0x1B00);
    s->oamFlags = ((17) << 6);
    s->graphics.size = 0;
    s->graphics.anim = 454;
    s->variant = 2;
...
{
    struct Task *t = TaskCreate(Task_Spikes_Down, sizeof(Spikes), 0x2000, 0, ((void*)0));
    Spikes *spikes = (void *)((t)->data);
    Sprite *s = &spikes->s;
    CamCoord worldX, worldY;
    ...
    s->palId = 0;
    s->hitboxes[0].index = -1;
    s->frameFlags = ((2) << (12)) | ((1) << (11));
    UpdateSpriteAnimation(s);
}

void Task_Spikes_Up(void)
{
    Spikes *spikes = (void *)((gCurTask)->data);
    Sprite *s = &spikes->s;
    MapEntity *me = spikes->base.me;
    CamCoord worldX, worldY;
    s32 i, j;

    worldX = ((spikes->base.meX)*8 + (spikes->base.regionX)*256);
    worldY = ((me->y)*8 + (spikes->base.regionY)*256);

    s->x = worldX - gCamera.x;
    s->y = worldY - gCamera.y;

    if ((gGameMode != 6) || (me->d.sData[0] != 0) || (gSpikesUnknownTimer != 0)) {
        i = 0;
        do {

            if (!(gStageFlags & 0x0080)) {
                sub_8020D44(s, me, spikes, &(*(((i) != 0) ? &gPartner : &gPlayer)));
            } else {
                sub_8020E98(s, me, spikes, &(*(((i) != 0) ? &gPartner : &gPlayer)));
            }
    ...
    if ((gGameMode == 6) && (me->d.sData[0] == 0) && (gSpikesUnknownTimer == 0)) {
        ...
        do {
            if (spikes->movestateBuffer[j] & 0x20) {
                (*(((j) != 0) ? &gPartner : &gPlayer)).moveState &= ~0x00000020;
            }

            if (spikes->movestateBuffer[j] & 0x8) {
                (*(((j) != 0) ? &gPartner : &gPlayer)).moveState &= ~0x00000008;
            }
...
void Task_Spikes_Down(void)
{
    ...
    if ((gGameMode == 6) && (me->d.sData[0] == 0) && (gSpikesUnknownTimer == 0)) {
        j = 0;
        do {
            if (spikes->movestateBuffer[j] & 0x00000020) {
                (*(((j) != 0) ? &gPartner : &gPlayer)).moveState &= ~0x00000020;
            }

            if (spikes->movestateBuffer[j] & 0x00000008) {
                (*(((j) != 0) ? &gPartner : &gPlayer)).moveState &= ~0x00000008;
            }
        } while (++j < gNumSingleplayerCharacters);
    }
...
void Task_Spikes_Horizontal(void)
{
    ...
    if ((gGameMode != 6) || (me->d.sData[0] != 0) || (gSpikesUnknownTimer != 0)) {
        i = 0;
        do {
            u32 res = Coll_Player_Platform(s, worldX, worldY, &(*(((i) != 0) ? &gPartner : &gPlayer)));
            s32 r2;
            ...
            if (res & 0x20000) {
                if (r2 == me->index) {
                    if (Coll_DamagePlayer(&(*(((i) != 0) ? &gPartner : &gPlayer)))) {
                        m4aSongNumStart(171);
                    }
                } else {
                    s8 arr[4] = { -((*(((i) != 0) ? &gPartner : &gPlayer)).spriteOffsetX + 5), -((*(((i) != 0) ? &gPartner : &gPlayer)).spriteOffsetY + 1), +((*(((i) != 0) ? &gPartner : &gPlayer)).spriteOffsetX + 5),
                                  +((*(((i) != 0) ? &gPartner : &gPlayer)).spriteOffsetY + 1) };

                    (*(((i) != 0) ? &gPartner : &gPlayer)).qWorldX = ((s32)((worldX + s->hitboxes[0].b.right - arr[0] + 1)*256));
                }
            } else if (res & 0x40000) {
                if ((r2 + 1) == me->index) {
                    if (Coll_DamagePlayer(&(*(((i) != 0) ? &gPartner : &gPlayer)))) {
                    ...
                    (*(((i) != 0) ? &gPartner : &gPlayer)).qWorldX = ((s32)((worldX + s->hitboxes[0].b.left - arr[2] - 1)*256));
                }
            }
        } while (++i < gNumSingleplayerCharacters);
    }
...
void Task_Spikes_HidingUp(void)
{
...

    {
        ...
        do {
            ...
            if (!(gStageFlags & 0x0080)) {
                res = sub_8020F2C(s, me, spikes, &(*(((i) != 0) ? &gPartner : &gPlayer)), &sp04);
            } else {
                res = sub_8021208(s, me, spikes, &(*(((i) != 0) ? &gPartner : &gPlayer)), &sp04);
            }
        } while (++i < gNumSingleplayerCharacters);
...
bool32 sub_8020D44(Sprite *s, MapEntity *me, Spikes *spikes, Player *p)
{
    ...
    if ((gGameMode == 6) && (me->d.sData[0] == 0) && (gSpikesUnknownTimer == 30)
        ...
        if (!(gStageFlags & 0x0080)) {
            p->qWorldY = ((s32)((worldY + s->hitboxes[0].b.top - arr[3])*256));
        } else {
            p->qWorldY = ((s32)((worldY + s->hitboxes[0].b.bottom + arr[3])*256));
        }
...
bool32 sub_8020F2C(Sprite *s, MapEntity *me, Spikes *spikes, Player *p, bool32 *out)
{
    ...
    if (sp0C < 60) {
        ...
        if (spikes->movestateBuffer[tempPlayerID] & 0x00000020) {
        ...
        if ((variant != 2) || ((pid != 0) && (*out))) {
            ...
            if (Coll_Player_Entity_Intersection(s, worldX, worldY, p) == 0x00080000) {
                s8 arr[4] = { -(p->spriteOffsetX + 5), (1 - p->spriteOffsetY), +(p->spriteOffsetX + 5), (p->spriteOffsetY - 1) };
    ...
    if (sp0C < 60) {
        ...
        if ((variant != 2) || ((pid != 0) && (*out))) {
            ...
            if (Coll_Player_Entity_Intersection(s, worldX, worldY, p) == 0x00080000) {
                ...
                if (!(gStageFlags & 0x0080)) {
                    p->qWorldY = ((s32)((worldY + s->hitboxes[0].b.bottom - arr[1])*256));
                } else {
                    p->qWorldY = ((s32)((worldY + s->hitboxes[0].b.top + arr[1])*256));
                }
