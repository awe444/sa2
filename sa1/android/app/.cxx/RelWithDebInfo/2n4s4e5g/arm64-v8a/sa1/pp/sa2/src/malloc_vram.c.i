# 1 "/home/awaszczak/git/sa2/sa1/../src/malloc_vram.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 384 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/awaszczak/git/sa2/sa1/../src/malloc_vram.c" 2
...
# 2 "/home/awaszczak/git/sa2/sa1/../src/malloc_vram.c" 2
...
# 3 "/home/awaszczak/git/sa2/sa1/../src/malloc_vram.c" 2
# 44 "/home/awaszczak/git/sa2/sa1/../src/malloc_vram.c"
void *VramMalloc(u32 numTiles)
{
    u16 i, j;
    u32 segCount = numTiles;
    segCount = (segCount + (((4 * 32) / 32) - 1)) / ((4 * 32) / 32);

    for (i = 0; i < gVramHeapMaxTileSlots / ((4 * 32) / 32); i++) {
        if (gVramHeapState[i] == 0) {
            for (j = 0; j < segCount; j++) {
                if (i + j >= (gVramHeapMaxTileSlots / ((4 * 32) / 32))) {
                    return ewram_end;
                }
            ...
            if (j == segCount) {
                ...
                return (void *)(gVramHeapStartAddr + i * (4 * 32));
            }
        } else {
            i = (gVramHeapState[i] - 1) + i;
        }
    }
    return ewram_end;
}
...
void VramResetHeapState(void) { { u16 tmp = (u16)(0); DmaSet(3, &tmp, gVramHeapState, (u32)((0x8000 | 0x0000 | 0x0000 | 0x0100 | 0x0000) << 16 | ((sizeof(gVramHeapState))/(16/8)))); }; }
...
void VramFree(void *addr)
{
    ...
    if (ewram_end != addr) {
        segmentId = (u32)(addr - gVramHeapStartAddr) / (4 * 32);
