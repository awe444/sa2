# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 384 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c" 2
...
# 2 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c" 2
...
# 3 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c" 2
...
# 4 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 6 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c" 2
...
# 7 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c" 2
...
# 8 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c" 2
...
# 9 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c" 2
...
# 10 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c" 2
...
u32 Coll_Player_Entity_Intersection(Sprite *s, CamCoord x, CamCoord y, Player *p);
# 11 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c" 2
...
# 13 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/mp_chao.c" 2
...
const u16 sChaoSpawnPositions[4][8][2] = {
    [(14 + 0) - 14] = {
        { 906, 290 },
        { 1250, 267 },
        { 1348, 700 },
...
struct Task *CreateMultiplayerChao(u8 spawnIndex, u8 id)
{
    ...
    spawnIndex %= (sizeof(sChaoSpawnPositions[0]) / sizeof((sChaoSpawnPositions[0])[0]));
    ...
    x = sChaoSpawnPositions[gCurrentLevel - 14][spawnIndex][0];
    if ((gCurrentLevel == (14 + 3)) && (spawnIndex > 4U)) {
        (s)->frameFlags |= (((1) << (11)));
        (s)->frameFlags |= ((2) << (12));
        y = sChaoSpawnPositions[gCurrentLevel - 14][spawnIndex][1] + 16;
    } else {
        y = sChaoSpawnPositions[gCurrentLevel - 14][spawnIndex][1] - 16;
    }
    ...
    chao->playerIdA = -1;
    chao->playerIdB = -1;
    chao->qWorldX = ((s32)((x)*256));
    chao->qWorldY = ((s32)((y)*256));
    chao->qDistanceToPlayer = 0;
    chao->angle = 0;

    s->x = ((int)((chao->qWorldX) >> 8)) - gCamera.x;
    s->y = ((int)((chao->qWorldY) >> 8)) - gCamera.y;
...
void Task_8028388(void)
{
    ...
    chao->playerIdB = chao->playerIdA;
    mpp = (void *)((gMultiplayerPlayerTasks[chao->playerIdA])->data);
    sprChao = &chao->s;
    ...
    if ((s16)worldX == ((s32)chao->qWorldX >> 8)) {
        ...
        if (charAnim == 1) {
            sprChao->graphics.anim = 703;
            sprChao->variant = 0;
        } else {
            sprChao->graphics.anim = 702;
            sprChao->variant = 0;
        }
    } else {
    ...
    sprChao->x = ((int)((chao->qWorldX) >> 8)) - gCamera.x;
    sprChao->y = (((int)((chao->qWorldY) >> 8)) - ((sinVal = (gSineTable[(theta)])) >> 11)) - gCamera.y;
...
void Task_8028518()
{
    ...
    if (UpdateChaoPosition(worldX, worldY)) {
        sprChao->graphics.anim = 704;
        sprChao->variant = 0;
        sprChao->x = ((int)((chao->qWorldX) >> 8)) - gCamera.x;
        sprChao->y = ((int)((chao->qWorldY) >> 8)) - gCamera.y;
    } else {
        sprChao->x = ((int)((chao->qWorldX) >> 8)) - gCamera.x;
        sprChao->y = (((int)((chao->qWorldY) >> 8)) - ((sinVal = (gSineTable[(theta)])) >> 11)) - gCamera.y;
    }
...
void Task_80286B0()
{
    ...
    chaoZero = (void *)((gChaoTasks[0])->data);
...
void sub_802888C()
{
    Chao *chao = (void *)((gCurTask)->data);
    Sprite *s = &chao->s;
    MultiplayerPlayer *mpp = (void *)((gMultiplayerPlayerTasks[chao->playerIdA])->data);

    if (chao->s.frameFlags & 0x400) {
        chao->qSpeedX = -((s32)((1.5)*256));
    } else {
        chao->qSpeedX = +((s32)((1.5)*256));
    }

    if (mpp->s.frameFlags & 0x800) {
        chao->qSpeedY = +((s32)((3.0)*256));
    } else {
        chao->qSpeedY = -((s32)((3.0)*256));
    }
...
void sub_8028910()
{
    ...
    if (mpp->s.frameFlags & 0x800) {
        res = sa2__sub_801F100(((int)((chao->qWorldY) >> 8)), ((int)((chao->qWorldX) >> 8)), (mpp->unk54 >> 7) % 2u, -8, sa2__sub_801EC3C);
        if (res < 0) {
            chao->qWorldY -= ((s32)((res)*256));
            s->graphics.anim = 698;
            s->variant = 2;
...
void Task_8028A1C()
{
    ...
    chao->s.x = ((int)((chao->qWorldX) >> 8)) - gCamera.x;
    chao->s.y = ((int)((chao->qWorldY) >> 8)) - gCamera.y;
...
bool32 UpdateChaoPosition(CamCoord playerX, CamCoord playerY)
{
    ...
    vecChaoPlayerX = playerX - ((int)((chao->qWorldX) >> 8));
    vecChaoPlayerY = playerY - ((int)((chao->qWorldY) >> 8));
    ...
    dirY = vecChaoPlayerY;
    if ((vecChaoPlayerX != 0) && (vecChaoPlayerY != 0)) {
        while ((((dirX) >= 0 ? (dirX) : -(dirX)) > 0x7F) || (((dirY) >= 0 ? (dirY) : -(dirY)) > 0x7F)) {
            dirX /= 2;
            dirY /= 2;
            if ((((dirX) >= 0 ? (dirX) : -(dirX)) <= 1) || ((dirY) >= 0 ? (dirY) : -(dirY)) <= 1) {
                break;
            }
        }
    }
    ...
    if ((((dirX) >= 0 ? (dirX) : -(dirX)) <= 1) && (((dirY) >= 0 ? (dirY) : -(dirY)) > 1)) {
        if (dirY > 0) {
            angle = ((int)((((double)(90)) / 360.0) * 1024));
        } else {
            angle = ((int)((((double)(270)) / 360.0) * 1024));
        }
    } else {
        if ((((dirY) >= 0 ? (dirY) : -(dirY)) <= 1) && (((dirX) >= 0 ? (dirX) : -(dirX)) > 1)) {
            if (dirX > 0) {
                angle = ((int)((((double)(0)) / 360.0) * 1024));
            } else {
                angle = ((int)((((double)(180)) / 360.0) * 1024));
            }
        } else {
            angle = (u16)sa2__sub_8004418(dirY, dirX);
        }
    }

    qDistance = ((s32)((Sqrt((vecChaoPlayerX * vecChaoPlayerX) + (vecChaoPlayerY * vecChaoPlayerY)))*256));

    if (qDistance > ((s32)((2.0)*256))) {
        qDistance = ((s32)((2.0)*256));
    } else if (qDistance == 0) {
        qDistance = 1;
    }
    ...
    if (chao->qDistanceToPlayer != 0) {
        chao->qWorldX += (chao->qDistanceToPlayer * (gSineTable[(angle) + 256])) >> 14;
        chao->qWorldY += (chao->qDistanceToPlayer * (gSineTable[(angle)])) >> 14;
    }
    return 0U;
}
