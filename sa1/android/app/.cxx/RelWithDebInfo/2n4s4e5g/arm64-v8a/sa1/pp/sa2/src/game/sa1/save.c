# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/save.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 384 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/save.c" 2
...
# 2 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/save.c" 2
# 1 "/home/awaszczak/git/sa2/sa1/../include/flags.h" 1
# 23 "/home/awaszczak/git/sa2/sa1/../include/flags.h"
extern u32 gFlags;
# 3 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/save.c" 2
...
void EwramFree(void *);
# 4 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/save.c" 2
...
# 5 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/save.c" 2
...
# 6 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/save.c" 2
...
# 37 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/save.c"
extern s8 __attribute__((aligned(4))) gUsedSaveSectorID;
...
bool32 SaveGameExists(void)
{
    ...
    {
        ReadFlash(sectorID, 0, (&gLoadedSaveGame), sizeof(struct SaveSectorData));

        if ((&gLoadedSaveGame)->security == 0x4F524950) {
            if ((&gLoadedSaveGame)->checksum == CalculateChecksum((&gLoadedSaveGame))) {
    ...
    if ((u8)gUsedSaveSectorID == 0xFF) {
        ...
        ReadFlash(gUsedSaveSectorID, 0, (&gLoadedSaveGame), sizeof(struct SaveSectorData));
        res = 1;
    }
...
u16 WriteSaveGame(void)
{
    ...
    if (gUsedSaveSectorID != -1) {
        ...
        for (i = 0; i < sizeof(struct SaveSectorData); i++) {
            ...
            u8 *dataIWRAM = (u8 *)(&gLoadedSaveGame) + i;
    ...
    if (result == 0) {
        (&gLoadedSaveGame)->security = 0x4F524950;
        (&gLoadedSaveGame)->checksum = CalculateChecksum((&gLoadedSaveGame));
        gUsedSaveSectorID = sectorID;

        result = ProgramFlashSectorAndVerifyNBytes(gUsedSaveSectorID, (&gLoadedSaveGame), sizeof((&gLoadedSaveGame)->security));
    }
    return result;
}
...
bool32 RegisterTimeRecord(TimeRecord newRecord)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        record = (&gLoadedSaveGame)->timeRecords.table[gSelectedCharacter][gCurrentLevel][i];

        if (newRecord < record) {
            tempRecord = record;
            (&gLoadedSaveGame)->timeRecords.table[gSelectedCharacter][gCurrentLevel][i] = newRecord;

            i2 = i;
            while (++i2 < 3) {
                (&gLoadedSaveGame)->timeRecords.table[gSelectedCharacter][gCurrentLevel][i2] ^= tempRecord; tempRecord ^= (&gLoadedSaveGame)->timeRecords.table[gSelectedCharacter][gCurrentLevel][i2]; (&gLoadedSaveGame)->timeRecords.table[gSelectedCharacter][gCurrentLevel][i2] = (tempRecord ^ (&gLoadedSaveGame)->timeRecords.table[gSelectedCharacter][gCurrentLevel][i2]);;
            }
...
u32 CalculateChecksum(void *data) { return ({ u32 j, checksum = 0; for (j = 0; j < __builtin_offsetof(struct SaveSectorData, checksum); j += sizeof(uintptr_t)) { checksum += *(uintptr_t *)((void *)(data) + j); } checksum; }); }
