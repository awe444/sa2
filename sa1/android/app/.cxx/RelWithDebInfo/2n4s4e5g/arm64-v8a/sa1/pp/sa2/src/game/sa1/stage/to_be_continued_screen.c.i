# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/to_be_continued_screen.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 384 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/to_be_continued_screen.c" 2
...
# 2 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/to_be_continued_screen.c" 2
...
# 3 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/to_be_continued_screen.c" 2
...
# 5 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/to_be_continued_screen.c" 2
...
# 7 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/to_be_continued_screen.c" 2
...
# 9 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/to_be_continued_screen.c" 2
# 1 "/home/awaszczak/git/sa2/sa1/../include/constants/sa1/vram_hardcoded.h" 1
# 10 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/to_be_continued_screen.c" 2
...
void CreateToBeContinuedScreen(void)
{
    ...
    s->graphics.dest = ((void *)&VRAM[0x10000] + 0x0000);
    s->oamFlags = ((18) << 6);
    s->graphics.size = 0;
    s->graphics.anim = 717;
    s->variant = 0;
    ...
    t2 = t;
    for (i = 0; i < (sizeof(tbc->tasks74) / sizeof((tbc->tasks74)[0])); i++) {
        tbc->tasks74[i] = t2 = sub_80125C0(t2, i + 1);
    }

    { u32 tmp = (u32)(((480 << 16) | (240 / 2))); DmaSet(3, &tmp, tbc->positions, (u32)((0x8000 | 0x0000 | 0x0400 | 0x0100 | 0x0000) << 16 | ((sizeof(tbc->positions))/(32/8)))); };

    gBgPalette[0] = ((0) | ((0) << 5) | ((0) << 10));
    gFlags |= 0x1;
}
...
void Task_ToBeContinuedScreenInit(void)
{
    ...
    tbc = (void *)((gCurTask)->data);
    tbc->unk7C = (tbc->unk7C + 8) & (1024 - 1);
    s = &tbc->s;

    s->x = ((((int)(((gSineTable[(tbc->unk7C) + 256])) >> 6)) * 15) >> 4) + (320 / 2);
...
void Task_80124C4(void)
{
    ToBeContinuedScreen *tbc = (void *)((gCurTask)->data);
    Sprite *s = &tbc->s;
...
struct Task *sub_80125C0(struct Task *tbcTask, u8 priority)
{
    ...
    tbc = (void *)((tbcTask)->data);
    shrunk->unk30 = (tbc->unk30 + 8) % 16u;

    s->x = tbc->s.x;
    s->y = tbc->s.y;

    s->graphics.dest = tbc->s.graphics.dest + (56 * 32);
    s->oamFlags = ((priority + 18) << 6);
    s->graphics.size = 0;
    s->graphics.anim = 717;
    s->variant = priority;
    s->animCursor = 0; s->qAnimDelay = ((s32)((0)*256)); s->prevVariant = -1; s->animSpeed = ((int)((float)(1.0)*0x10)); s->palId = 0; s->hitboxes[0].index = -1;;
    ...
    { u32 tmp = (u32)(((480 << 16) | (240 / 2))); DmaSet(3, &tmp, shrunk->positions, (u32)((0x8000 | 0x0000 | 0x0400 | 0x0100 | 0x0000) << 16 | ((sizeof(shrunk->positions))/(32/8)))); };
...
void Task_ShrunkToBeContinuedScreenInit(void)
{
    ToBeContinuedScreenShrunk *sub = (void *)((gCurTask)->data);
    ToBeContinuedScreen *tbc = (void *)(((struct Task *)((gCurTask)->parent))->data);
