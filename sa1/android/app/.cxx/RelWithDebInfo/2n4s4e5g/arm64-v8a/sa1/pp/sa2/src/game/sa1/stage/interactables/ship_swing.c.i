# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ship_swing.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 384 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ship_swing.c" 2
...
# 2 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ship_swing.c" 2
...
# 3 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ship_swing.c" 2
...
# 4 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ship_swing.c" 2
...
u32 Coll_Player_Entity_Intersection(Sprite *s, CamCoord x, CamCoord y, Player *p);
# 6 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ship_swing.c" 2
...
# 10 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ship_swing.c" 2
...
void CreateEntity_ShipSwing(MapEntity *me, u16 regionX, u16 regionY, u8 id)
{
    ...
    sprShip->x = ((me->x)*8 + (regionX)*256);
    sprShip->y = ((me->y)*8 + (regionY)*256);

    (me)->x = (-2);;

    sprShip->graphics.dest = VramMalloc((((96 / 2) / 8) * (40 / 8)));
    sprShip->oamFlags = ((4) << 6);
    sprShip->graphics.size = 0;
    sprShip->graphics.anim = 491;
    sprShip->variant = 0;
    sprShip->animCursor = 0;
    sprShip->qAnimDelay = ((s32)((0)*256));
    sprShip->prevVariant = -1;
    sprShip->animSpeed = ((int)((float)(1.0)*0x10));
    sprShip->palId = 0;
    sprShip->hitboxes[0].index = -1;
    sprShip->frameFlags = ((2) << (12));

    sprHook->graphics.dest = VramMalloc(((24 / 8) * (24 / 8)));
    sprHook->oamFlags = ((18) << 6);
    sprHook->graphics.size = 0;
    sprHook->graphics.anim = 490;
    sprHook->variant = 0;
    sprHook->animCursor = 0;
    sprHook->qAnimDelay = ((s32)((0)*256));
    sprHook->prevVariant = -1;
    sprHook->animSpeed = ((int)((float)(1.0)*0x10));
    sprHook->palId = 0;
    sprHook->hitboxes[0].index = -1;
    sprHook->frameFlags = ((2) << (12));

    sprSegment->graphics.dest = VramMalloc(((8 / 8) * (8 / 8)));
    sprSegment->oamFlags = ((18) << 6);
    sprSegment->graphics.size = 0;
    sprSegment->graphics.anim = 489;
    sprSegment->variant = 0;
    sprSegment->animCursor = 0;
    sprSegment->qAnimDelay = ((s32)((0)*256));
    sprSegment->prevVariant = -1;
    sprSegment->animSpeed = ((int)((float)(1.0)*0x10));
    sprSegment->palId = 0;
    sprSegment->hitboxes[0].index = -1;
    sprSegment->frameFlags = ((2) << (12));
...
void Task_ShipSwing(void)
{
    ...
    worldX = ((swing->base.meX)*8 + (swing->base.regionX)*256);
    worldY = ((me->y)*8 + (swing->base.regionY)*256);

    worldX2 = worldX;
    worldY2 = worldY;

    modRes = ({((0x200) != 0) ? ((s32)(gStageTime * 2) % (s32)(0x200)) : 0;});

    if (modRes > 0x100) {
        modRes -= 0x100;
        modRes = 0x100 - modRes;

        r3 = ((gSineTable[(modRes * 2)]) >> 8);
    } else {
        r3 = -((gSineTable[(modRes * 2)]) >> 8);
    }

    sinRes = (gSineTable[(((modRes - 0x7E) & 0x1FF) * 2)]);
    theta = ((sinRes >> 7) & 0x1FF);
    theta *= 2;
    dirX = (gSineTable[(theta)]);
    dirY = (gSineTable[(theta) + 256]) + r3;
    ...
    if (((worldX2) > gCamera.x + (320 + (256 / 2)) || (worldX2) < gCamera.x - (256 / 2) || (worldY2) > gCamera.y + ((240) + ((256) / 2)) || (worldY2) < gCamera.y - (256 / 2)) && (((u16)(sprShip->x + (256 / 2)) > 320 + 256) || (sprShip->y + (256 / 2) < 0) || (sprShip->y > 240 + (256 / 2)))) {
        { me->x = swing->base.meX; };
    ...
    for (j = 0; j < 5; j++) {
        dtDirX += ({((100) != 0) ? ((s32)(dirX * 110) / (s32)(100)) : 0;});
        dtDirY += ({((100) != 0) ? ((s32)(dirY * 110) / (s32)(100)) : 0;});

        sprSegment->x = worldX - gCamera.x + (dtDirX >> 10);
        sprSegment->y = worldY - gCamera.y + (dtDirY >> 10);

        if (j == 5 - 1) {
            sprSegment->oamFlags = ((3) << 6);
        } else {
            sprSegment->oamFlags = ((18) << 6);
        }
    ...
    do {
        if (!((*(((i) != 0) ? &gPartner : &gPlayer)).moveState & 0x00000080)) {
            if (((s32)((worldY - 4)*256)) > (*(((i) != 0) ? &gPartner : &gPlayer)).qWorldY) {
                if (Coll_Player_PlatformCrumbling(sprHook, worldX, worldY, &(*(((i) != 0) ? &gPartner : &gPlayer))) & 0x00000008) {
                    if (!(((swing->unkA7) >> (i)) & 1)) {
                        (swing->unkA7) |= (1 << (i));
                    }
                } else {
            ...
            if (swing->unkA5[i] > 0) {
                ...
                if (Coll_Player_PlatformCrumbling(sprShip, (dtDirX >> 10) + worldX, (dtDirY >> 10) + worldY, &(*(((i) != 0) ? &gPartner : &gPlayer))) & 0x00000008) {
                    if (!(((swing->unkA4) >> (i)) & 1)) {
                        (swing->unkA4) |= (1 << (i));
                    } else {
                        (*(((i) != 0) ? &gPartner : &gPlayer)).qWorldX += ((s32)(((worldX + (dtDirX >> 10)) - swing->unk9C[i])*256));
                        (*(((i) != 0) ? &gPartner : &gPlayer)).qWorldY = ((s32)(((worldY - 5 + (dtDirY >> 10)))*256));
                    }
