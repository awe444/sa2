# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ferris_wheel.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 384 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ferris_wheel.c" 2
...
# 2 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ferris_wheel.c" 2
...
# 3 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ferris_wheel.c" 2
...
# 4 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ferris_wheel.c" 2
...
void m4aMPlayPitchControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s16 pitch);
void m4aMPlayPanpotControl(struct MP2KPlayerState *mplayInfo, u16 trackBits, s8 pan);
# 6 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ferris_wheel.c" 2
...
u32 Coll_Player_Entity_Intersection(Sprite *s, CamCoord x, CamCoord y, Player *p);
# 7 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ferris_wheel.c" 2
...
# 10 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ferris_wheel.c" 2
...
# 12 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ferris_wheel.c" 2
# 1 "/home/awaszczak/git/sa2/sa1/../include/constants/sa1/songs.h" 1
# 13 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ferris_wheel.c" 2
# 1 "/home/awaszczak/git/sa2/sa1/../include/constants/sa1/vram_hardcoded.h" 1
# 14 "/home/awaszczak/git/sa2/sa1/../src/game/sa1/stage/interactables/ferris_wheel.c" 2
...
const u16 gUnknown_086CEE2C[18] = {
    463, 463, 506, 506,
    495, 495, 523, 523,
    535, 535, 517, 598,
    517, 517, 463, 506,
...
void CreateEntity_FerrisWheel(MapEntity *me, u16 regionX, u16 regionY, u8 id)
{
    ...
    s3 = &wheel->s3;
    s3->graphics.dest = (void *)(&VRAM[0x10000] + 0x3020);
    s3->oamFlags = ((18) << 6);
    s3->graphics.size = 0;
    s3->graphics.anim = 530;
    s3->variant = 0;
    s3->animCursor = 0;
    ...
    s2 = &wheel->s2;
    s2->graphics.dest = (void *)(&VRAM[0x10000] + 0x30A0);
    s2->oamFlags = ((18) << 6);
    s2->graphics.size = 0;
    s2->graphics.anim = 531;
    s2->variant = 0;
    s2->animCursor = 0;
...
void Task_FerrisWheel(void)
{
    ...
    if (wheel->unkB2) {
        wheel->unkA4 = (gSineTable[(((+me->d.sData[1] * gStageTime + wheel->unk9C) & 0x7FF) >> 1)]) * wheel->unkB3;
        wheel->unkA8 = (gSineTable[(((+me->d.sData[1] * gStageTime + wheel->unk9C) & 0x7FF) >> 1) + 256]) * wheel->unkB3;
    } else {
        wheel->unkA4 = (gSineTable[(((-me->d.sData[1] * gStageTime + wheel->unk9C) & 0x7FF) >> 1)]) * wheel->unkB3;
        wheel->unkA8 = (gSineTable[(((-me->d.sData[1] * gStageTime + wheel->unk9C) & 0x7FF) >> 1) + 256]) * wheel->unkB3;
    }
    ...
    do {
        if ((((wheel->unkB6) >> (i)) & 1)) {
            if (gStageTime & 0x1) {
                (*(((i) != 0) ? &gPartner : &gPlayer)).qWorldX += ((wheel->unkA4 - wheel->unkA0) >> 3) + 1;
            } else {
                (*(((i) != 0) ? &gPartner : &gPlayer)).qWorldX += ((wheel->unkA4 - wheel->unkA0) >> 3) + 0;
            }
        }

        if (!((*(((i) != 0) ? &gPartner : &gPlayer)).moveState & 0x00000080)) {
            Coll_Player_PlatformCrumbling(s, worldX + sp14, worldY + sp18, &(*(((i) != 0) ? &gPartner : &gPlayer)));

            if (((*(((i) != 0) ? &gPartner : &gPlayer)).moveState & 0x00000008) && (*(((i) != 0) ? &gPartner : &gPlayer)).stoodObj == s) {
                (wheel->unkB6) |= (1 << (i));

                wheel->qUnkB0 += ((s32)((0.0625)*256));

                if (wheel->qUnkB0 > ((s32)((1.0)*256))) {
                    wheel->qUnkB0 = ((s32)((1.0)*256));
                }

                if ((*(((i) != 0) ? &gPartner : &gPlayer)).moveState & (0x00000400 | 0x00000004)) {
                    (*(((i) != 0) ? &gPartner : &gPlayer)).qWorldY = ((s32)(((worldY + sp18) - 15)*256));
                } else {
                    (*(((i) != 0) ? &gPartner : &gPlayer)).qWorldY = ((s32)(((worldY + sp18) - 20)*256));
                }

            } else {
                (wheel->unkB6) &= ~(1 << (i));

                if (wheel->qUnkB0 > ((s32)((0.0625)*256))) {
                    wheel->qUnkB0 -= ((s32)((0.0625)*256));
                } else {
                    wheel->qUnkB0 = ((s32)((0)*256));
                }
            }
        } else {
            if ((((wheel->unkB6) >> (i)) & 1)) {
...

    {
        ...
        s->y += ({((2500) != 0) ? ((s32)((gSineTable[(wheel->qUnkB0)])) / (s32)(2500)) : 0;});
        ...
        if ((((gCurrentLevel) >> 1) != 2) && (gCurrentLevel != (14 + 2))) {
            ...
            sp14 = ({((3) != 0) ? ((s32)(wheel->unkA4) / (s32)(3)) : 0;});
            sp18 = ({((3) != 0) ? ((s32)(wheel->unkA8) / (s32)(3)) : 0;});
